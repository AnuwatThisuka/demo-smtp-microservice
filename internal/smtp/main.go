...
func Ping() error {
	...
	log.Println("SMTP server is ready to accept connections")

	return nil
}

func getAddr() string {
	return fmt.Sprintf("%s:%d", config.MainConfig.SMTP.Host, config.MainConfig.SMTP.Port)
}

func getAuth() smtp.Auth {
	return smtp.PlainAuth("", config.MainConfig.SMTP.Username, config.MainConfig.SMTP.Password, config.MainConfig.SMTP.Host)
}
